...
router.post('/passwordless/email/verify/code', async (req, res, next) => {
  try {
    ...
    const { value, error } = schema.validate(req.body);
    if (error) throw new AppError('Validation error', 400, error.details);
  const { user, verifyResponse, firebaseToken } = await verifyWithCode(value.code, value.authRequestId);
  req.session.user = user;
  res.json({ user, firebaseToken, verify: verifyResponse });
  } catch (err) {
    next(err);
  }
});
...
router.get('/passwordless/verify', async (req, res, next) => {
  try {
    ...
    if (error) throw new AppError('Validation error', 400, error.details);
    let user, verifyResponse, firebaseToken;
    try {
      ({ user, verifyResponse, firebaseToken } = await verifyWithLink(value.link_token, value.auth_request_id));
    } catch (e) {
      const msg = (e && e.details && e.details.sdkError) || e.message || '';
      ...
      if (value.auth_request_id && /auth request expired/i.test(msg)) {
        ({ user, verifyResponse, firebaseToken } = await verifyWithLink(value.link_token, undefined));
      } else {
        throw e;
      }
    }
